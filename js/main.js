...
function init() {
  ...
  var copyKeyElement = document.getElementById('copyKey');
  copyKeyElement.addEventListener('click', function sendKeyHandler() {

    inputContext = navigator.mozInputMethod.inputcontext;
    if (inputContext) {
      console.log("inputContext is defined!");
      var promise = inputContext.getText();
      promise.then(function(text){
        localStorage.copypaste = text;
        console.log("Promise done: "+localStorage.copypaste);

      }, function(err){
        console.log("Promise not done!");
      });
    }else{
      console.log("inputContext is not defined!");
    }

  });

  var pasteKeyElement = document.getElementById('pasteKey');
  pasteKeyElement.addEventListener('click', function sendKeyHandler() {
    var testString = localStorage.copypaste;
    console.log("Pasted: "+testString);
